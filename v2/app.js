...
function results(){
 const ranked=R.rankHotels(hotels,profile),n=R.nights(profile),limit=R.shortlistLimit(profile.maxShortlist),ages=profile.travellers.map(t=>Number(t.age)||0).join(', ');
 $('profileSummary').textContent=`${profile.travellers.length} ос. (${ages} р.) · ${R.groupType(profile)==='family'?'сімейний':'дорослий'} профіль · ${dateSummary()}${n?` · ${n} ночей`:''} · ${Number(profile.budgetPerNight).toLocaleString('uk-UA')} грн/ніч`;
...
Promise.all([fetch('profile.json',{cache:'no-store'}).then(r=>r.json()),A.loadHotelBase()]).then(([p,h])=>{base=p;base.version=2;base.preferences={quiet:5,coziness:4,spa:5,waterparks:0,pools:4,amusement:0,nature:5,mountains:4,active:2,calm:5,kids:0,culture:2,entertainment:1,...base.preferences};hotels=h.hotels;profile=load();profile.date={...base.date,...profile.date};profile.preferences={...base.preferences,...profile.preferences};if(!Array.isArray(profile.travellers)||!profile.travellers.length)profile.travellers=structuredClone(base.travellers);profile.travellers=profile.travellers.slice(0,5).map((t,i)=>({label:`Мандрівник ${i+1}`,age:Math.max(0,Math.min(120,Number(t?.age)||0))}));document.body.dataset.travel2='ready';draw()}).catch(e=>{$('status').textContent='Помилка завантаження: '+e.message});
